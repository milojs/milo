...
const eventsSplitRegExp = /\s*(\,|\s)\s*/;


function Messenger(hostObject, proxyMethods, messageSource) {
	check(hostObject, Object);
 	check(proxyMethods, Match.Optional(Match.ObjectHash(Function)));
 	check(messageSource, Match.Optional({
 	...
 	if (proxyMethods)
 		_.eachKey(proxyMethods, function(methodName, proxyMethodName) {
