...
async function onDragStart(eventType, event) {
    event.stopPropagation();
    ...
    await setAdditionalDataTypes.call(this);
    ...
    async function setAdditionalDataTypes() {
        if(!this.config.dataTypes) return;
        const dataTypes = Object.entries(this.config.dataTypes).map(([dataType, getDataFunc]) => {
            if (typeof getDataFunc == 'string') getDataFunc = owner[getDataFunc];
            return Promise.resolve(getDataFunc.call(this.owner, dataType)).then(data => {
                if (typeof data == 'object') data = JSON.stringify(data);
                if (data) dt.setData(dataType, data);
                return data;
            });
        });
        return await Promise.all(dataTypes);
    }
}
